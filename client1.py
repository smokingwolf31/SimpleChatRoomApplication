    ...
    while myAccount.status == account.Status.ONLINE:
        try:
            ...
            if readable:
                ...
                # Adjust for time zone difference (+2 hours)
                adjustedTime = datetime.now() + timedelta(hours=2)
                timestamp = adjustedTime.strftime('%Y-%m-%d %H:%M:%S')  # Get adjusted timestamp
                
                # Append timestamp to message
                formattedMessage = f"[{timestamp}] {sender}: {messageSent}"
                ...
                if messageType == "Group**":
                    ...
                    if groupName == myAccount.currentlyInbox:
                        ...
                        print("Enter your message or Back* to exit\n\n")
                        printGroupInbox(groupName)
                        print(formattedMessage)  # Print with timestamp
                        myAccount.groupInbox.setdefault(groupName, [0]).append(formattedMessage)  # Store with timestamp
                    ...
                    if(sender == myAccount.currentlyInbox):
                        ...
                        print(formattedMessage)  # Print with timestamp
                        myAccount.privateInbox.setdefault(sender, [0]).append(formattedMessage)  # Store with timestamp
